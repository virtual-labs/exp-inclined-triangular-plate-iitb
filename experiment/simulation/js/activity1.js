...
function activity1() {
    ...
    <div style='margin-top: 2vw;'>
        ...
        <div class="fs-16px">
        <p style="text-align: center">Hydrostatic force on inclined triangular plate</p>
        </div>
...
function start_act1() {
    ...
    <div class='collapse divide' style='style='margin-top: 2vw; 'width: 80%; margin: auto;' id='tb1-box'>
        <p style="font-weight: 400; font-size: 18px;">
            A triangular plate of b=${b} m and h=b is y=${y} m below the free surface of the water and inclining an angle of ${angle}<sup>o</sup> with the free surface of the water. Find the total pressure force and its position.
        </p>
        ...
        <div class="row">
            ...
            <div class="col-4">
                <p style="font-weight: 400; font-size: 18px;">
                    $$ \\bar{h} = \\frac{h \\sin\\theta}{3} + y \\quad m $$
                </p>
            </div>
            <div class="col-6"></div>
...
function calculation() {
    let B = (Math.random() * 2) + 2;
    b = parseFloat(B.toFixed(1));
    h = b;
    let Y = (Math.random() * 0.5) + 1.5;
    y = parseFloat(Y.toFixed(1));
    angle = Math.floor(Math.random() * (15 + 1)) + 30;
    console.log("b = ", b);
    console.log("h = ", h);
    console.log("y = ", y);
    console.log("angle = ", angle);
    h_bar = (h * (Math.sin(((Math.PI) / 180) * angle)) / 3) + y;
    console.log("h bar = ", parseFloat(h_bar.toFixed(2)));
    area = (b * h) / 2;
    console.log("area = ", area);
    f = rho * gravity * area * h_bar;
    console.log("f= ", f);
    ig = (b * Math.pow(h, 3)) / 36;
    console.log("ig= ", ig);
    h_star = ((ig * Math.pow((Math.sin(((Math.PI) / 180) * angle)), 2)) / area * h_bar) + h_bar;
...
function verify_area() {
    ...
    a_inp.remove();
    sp2.innerText = `${parseFloat(area.toFixed(2))}`;
    //btn.remove();
    if (btn) {
        btn.remove();
    }
